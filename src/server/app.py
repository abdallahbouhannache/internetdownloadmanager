    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(file_url,headers=headers) as response:
            ...
            while file_size_on_disk < file_size :
                ...
                task = asyncio.create_task(write_chunk_to_file(save_path,data,file_name,save_state_file,file_size_on_disk))
                tasks.append(task)

                # task = asyncio.ensure_future(write_chunk_to_file(save_path,data))
...
def resume_download():
    ...
    return "Download resumed successfully"
